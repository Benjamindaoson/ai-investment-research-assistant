...
import com.itzixi.config.NotifyChannelConfig;
import com.itzixi.service.RssService;
import com.itzixi.service.StockService;
import com.itzixi.service.WechatBotService;
...
public class RssServiceImpl implements RssService {
    ...
    @Resource
    private WechatWorkBotService wechatWorkBotService;

    @Resource
    private NotifyChannelConfig notifyChannelConfig;
    ...
    public void displayRss() throws Exception {
        ...
        for (SyndEntry entry : rssList) {
            ...
            if (transEntity != null) {
                ...
                if (transResultList != null && !transResultList.isEmpty()) {
...
//                throw new RuntimeException(e);
            ...
            log.info("新股票异动: {}", stockMsg);

            stockMsgList.add(stockMsg);
        }

        if (!stockMsgList.isEmpty()) {
            if (notifyChannelConfig.isDingtalk()) {
                dingTalkApi.sendTextMessage(dingTalkApi.formatStockInfoFromList(stockMsgList));
            }
            if (notifyChannelConfig.isWechat()) {
                try {
                    wechatBotService.sendMessage(stockMsgList);
                } catch (Exception e) {
                    throw new RuntimeException(e);
                }
            }
            if (notifyChannelConfig.isWecom()) {
                wechatWorkBotService.sendMessage(stockMsgList);
            }
        }
